...
class EventLoop(object):
    ...
    def schedule(self, coroutine, value=None, stack=(), when=None):
        ...
        stack containing the coroutines that are waiting for the value yielded by this coroutine.
        """
...
def print_every(message, interval):
    """
    Coroutine task to repeatedly print the message at the given interval(in seconds)
    """
    while True:
...
def read_input(loop):
    """
    Coroutine task to repeatedly read new lines of input from stdin, tread
...
def main():
    loop = EventLoop()
    hello_task = print_every('Helloworld', 3)
